...
def good_function():
    ...
    if(count_1_of_array > M) and (count_1_of_array < N):
        ...
        return str(digit) + '_' + str(random_number_system) + str(' ')
...
for i in digits_all:
    if (''.join([str(elem) for elem in i]).count(' ')) != 1:
        _suitable_digits.append(''.join([str(elem) for elem in i]))

digits_all = str(digits_all)
digits_all = digits_all.strip('[')
digits_all = digits_all.strip(']')

suitable_digits = str(_suitable_digits)
Col_vo_True_Digit = int(suitable_digits.count('_'))
